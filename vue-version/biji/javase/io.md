...
`java.io.File` 是 Java 里**专门用来操作 文件和文件夹路径**的类

Java File 类：用来描述文件 / 文件夹路径，做增删查判，不负责读写文件内容。
...
File 对象**只是代表文件 / 文件夹的路径**，**不读取文件内容**，只做：判断是否存在、创建、删除、获取文件名、遍历文件夹 等操作。

## 2.File 类能干什么

1. 表示**文件**路径（txt、java、图片等）
2. 表示**文件夹**路径
3. 判断文件 / 文件夹是否存在
4. 创建文件、创建单层 / 多层文件夹
5. 删除文件、空文件夹
...
读写内容要用：IO 流（字节流、字符流），File 只负责 “找门牌号”，不负责 “进屋拿东西”。
...
| `exists()`          | 判断文件 / 文件夹是否存在  |
| `isFile()`          | 判断是不是**文件**         |
| `isDirectory()`     | 判断是不是**文件夹**       |
| `createNewFile()`   | 创建**空文件**             |
| `mkdir()`           | 创建**单层**文件夹         |
| `mkdirs()`          | 创建**多层**文件夹（常用） |
| `delete()`          | 删除文件 / 空文件夹        |
| `getName()`         | 获取名称                   |
...
| `length()`          | 获取文件大小（字节）       |
| `listFiles()`       | 遍历文件夹下所有 File 对象 |

## 5.File的静态成员

`java.io.File` 里**四个常用静态常量**，作用：**跨平台分隔符**，写路径不用自己写 `/` 或 `\`，适配 Windows / Linux / Mac。
...
Windows：;
Linux/Mac：:
4. public static final char pathSeparatorChar
路径分隔符（字符形式）
```
...
File 构造方法**只是封装路径**，**不会创建真实文件**（要 `createNewFile()` 才会创建）
...
`mkdirs()`：可以创建**多级**嵌套文件夹，开发首选

`createNewFile()`：只能创建**空文件**，不能建文件夹

`delete()`：只能删**文件**或**空文件夹**，有内容的文件夹删不掉

File 类**只操作路径和文件属性**，**不读写文件里面的内容**（读写要用 IO 流）
...
public class Demo04 {
    public static void main(String[] args) {
        ...
        // ========== 4. 遍历文件夹下所有文件 ==========
        File parentDir = new File("D:/javaTest");
...
![](https://zhuxiaoyi1.oss-cn-shenzhen.aliyuncs.com/img/io流向.png)
...
![](https://zhuxiaoyi1.oss-cn-shenzhen.aliyuncs.com/img/字节流复制图片.png)
...
| 对比维度     | 字节流 ByteStream                                 | 字符流 CharStream                |
| ------------ | ------------------------------------------------- | -------------------------------- |
| 处理单位     | **字节（1Byte）**                                 | **字符（2 字节，按编码）**       |
| 底层原理     | 直接操作二进制数据                                | 底层依旧是字节，**自带编码解码** |
| 基类抽象父类 | InputStream、OutputStream                         | Reader、Writer                   |
| 适用文件类型 | **所有文件**：图片、视频、音频、压缩包、exe、文档 | **纯文本文件**：txt、java、md 等 |
| 是否有缓冲区 | 无自带缓冲区，直接读写                            | **自带缓冲区**，不刷新不写入     |
| 中文乱码     | 不会乱码（原样二进制拷贝）                        | 编码不匹配容易乱码               |
| 关闭 / 刷新  | 直接 close 即可                                   | 需要 **flush () 刷新缓冲区**     |
| 读写方式     | 字节数组 byte []                                  | 字符数组 char [] / 字符串 String |
| 拷贝推荐     | 做文件复制、媒体文件必用                          | 只用来**读写字符文本**           |